...
const index = (props) => {
    ...
    const { fileUrls, setFileUrls, isBusy, path,
        multiple = true,
    ...
    const fileParams = ({ meta, file }) => {
        // return { url: `${NEXT_URL}/api/file/upload` }


        const body = new FormData()
        body.append('path', path || '')
        body.append('file', file)
        return { url: `${NEXT_URL}/api/file/upload`, body }
    ...
    const selectFileInput = ({ accept, onFiles, files, getFilesFromEvent, extra: { maxFiles }, }) => {

        return (

            <label className='flex flex-col cursor-pointer items-center justify-center'>
                <svg className="flex-1 w-8 h-8 mb-4  text-gray-500 dark:text-gray-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 16">
    ...
    const layoutComponent = (props) => {
        ...
        return (
            <div {...dropzoneProps} title='drag & drop here' className='flex flex-col items-center justify-center w-full min-w-56 h-56 border-2 border-gray-300 border-dashed rounded-lg  bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600'>
                {input}
