...
const SelectOne = (props) => {
    const items = domStyleProperties,
        idField = "name",
        textField = "name",
        descriptionField = "description";
    const { setSelectedValue, selectedValue, buttonProps, menuListClassName, ...otherProps } = props
    //  const [selectedValue, setSelectedValue] = React.useState(null);
    const ChevronDownIcon = ({ className }) => {
        return (<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`w-3 h-3 ${className || ''}`}>
    ...
    return (
        <Menu placement="bottom-start" open={openMenu} ref={componentRef}
            handler={setOpenMenu}>
            <MenuHandler>
                <div className={"bg-gray-100 shadow-sm rounded-l-full border px-2 text-xs h-6 flex flex-row justify-between items-center w-40"} title={getDesc(selectedValue)} ><span>{getText(selectedValue)}</span>
                    <ChevronDownIcon
                        strokeWidth={2.5}
                        className={`h-3.5 w-3.5  transition-all delay-600 ease-in-out ${openMenu ? "rotate-180" : ""}`}
                    /></div>
            </MenuHandler>
            <MenuList className={`px-1 py-0 max-h-[400px] overflow-y-auto`} >
                {
                    items.map(item => {
