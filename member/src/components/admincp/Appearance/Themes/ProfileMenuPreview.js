        ...
        <Menu placement="bottom-end" >
            <MenuHandler>
                ...
                <div className='flex-none animate-pulse'>
                    ...
                    <div className=" cursor-pointer relative inline-flex items-center justify-center w-9 h-9 overflow-hidden bg-gray-100 rounded-full border-1 dark:bg-gray-100  dark:border-gray-400   border-gray-900">
                        <span className="font-bold text-sm text-gray-900 dark:text-gray-300"></span>
                    </div>
                </div>


            </MenuHandler>
            <MenuList className="p-0 m-0 w-80 border-0 rounded-r-none shadow-2xl ">

                <div className="cursor-pointer h-28 pt-12 bg-gradient-to-b outline-none from-gray-300  to-gray-900 px-3 " >
                    <Typography variant="lead" className="font-bold text-white uppercase ">
                        &nbsp;
                    </Typography>
                    <Typography variant="small" className="font-thin capitalize text-white " >
                        &nbsp;
                    </Typography>
                </div>


                <MenuItem className="flex items-center gap-2 rounded-none animate-pulse " >
                    <svg
                        width="16"
                        height="16"
                        viewBox="0 0 16 16"
                    ...
                    <Typography variant="small" className="font-medium bg-gray-300 rounded-lg w-full" >
                        &nbsp;
                    </Typography>
                </MenuItem>

                <MenuItem className="flex items-center gap-2  rounded-none">
                    <svg
                        width="14"
                        height="14"
                        viewBox="0 0 14 14"
                ...
                <div className="shadow-md cursor-pointer absolute top-20 right-0 inline-flex items-center justify-center w-9 h-9 m-4 p-1 overflow-hidden bg-gray-300 rounded-full border-1 dark:bg-gray-100  dark:border-gray-400   border-gray-900">
...
export default function ProfileMenuPreview({ user, logout }) {
    const router = useRouter();

    return (<>
        {user ? (<Menu placement="bottom-end scale-75" >
            <MenuHandler>
            ...
            <MenuList className="p-0 m-0 w-80 border-0 rounded-tr-none shadow-m shadow-gray-600 flex flex-col scale-75">

                <div className="cursor-pointer h-28 pt-12 bg-gradient-to-b outline-none from-gray-300  to-gray-900 px-3" onClick={(e) => { e.preventDefault();  return false; }}>
                    <Typography variant="lead" className="font-bold text-white uppercase">
                        {user.profile.displayName.length > 0 ? user.profile.displayName : (user.name ? user.name : `${user.profile.firstName}`)}
                    </Typography>
                    <Typography variant="small" className="font-thin capitalize text-white">
                        {user.userGroup ? user.userGroup.name : user.role}
                    </Typography>
                </div>
                <div onClick={(e) => { e.preventDefault(); router.push('/user/profile/edit'); return false; }} className="shadow-md cursor-pointer absolute top-20 right-0 inline-flex items-center justify-center w-9 h-9 m-4 p-1 overflow-hidden bg-orange-300 rounded-full border-1 dark:bg-gray-100  dark:border-gray-400   border-gray-900">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                    </svg>

                </div>
                <MenuItem className="flex items-center gap-2 rounded-none" onClick={(e) => { e.preventDefault();  return false; }}>
                    <svg
                        width="16"
                        height="16"
                        viewBox="0 0 16 16"
                ...
                {user.userGroup && user.userGroup.name == "Administrator" &&
                    <MenuItem className="flex items-center gap-2 rounded-none" onClick={() => {  }}>
