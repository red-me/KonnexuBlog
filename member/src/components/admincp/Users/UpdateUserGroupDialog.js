...
function UpdateUserGroupDialog(props) {
    const { id, visible, setVisible } = props

    const { updateUserGroup, userGroups, isLoading, isError, clearError } = useContext(UserContext)
    ...
    const closeDialog = () => { setVisible(false); clearError() }

    useEffect(() => {
        if (id !== null) {
            const userGroup = userGroups.defaultUserGroupList.find(d => d.id == id) || userGroups.customUserGroupList.find(d => d.id == id)
    ...
    const save = () => {
        ...
        if (updateUserGroup({ id, name: userGroupName })) {
