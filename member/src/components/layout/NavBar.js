...
export default function NavBar() {
  const { theme, DEFAULT_THEME, ThemeSwitcher } = useContext(AppThemeContext)
  const { menus } = useContext(AppearanceContext)

  const [headerMenu, setHeaderMenu] = useState(null)

  useEffect(() => {
    if (menus && menus.find(m => m.name == "main")) {
      setHeaderMenu(menus.find(m => m.name == "main"))
    }
  }, [menus])

  const navStyle = theme.data.navigation.style
  const navClassName = theme.data.navigation.className
  const navListClassName = theme.data.navigation_list.className
  const navListStyle = theme.data.navigation_list.style
  const itemStyle = theme.data.navigation_item.style
  const itemTextStyle = theme.data.navigation_item_text.style
  const itemIconClassName = theme.data.navigation_item_icon.className
  ...
  const navList = (
    <ul className="mt-2 mb-4 flex flex-col gap-1 lg:mb-0 lg:mt-0 lg:flex-row lg:items-center  uppercase font-extrabold ">
      {headerMenu && headerMenu.data.map(menuItem => {

        const isCurrent = currentPath.toLowerCase() == menuItem.url.toLowerCase()
        return <Typography
          as="li"
          variant="small"

          className={`flex flex-row items-center p-1 font-medium  transition-all duration-300  ${isCurrent ? 'opacity-95' : 'opacity-85'}  hover:opacity-100`}
        >


          <Link href={menuItem.url} className={`flex flex-row gap-1 items-center ${isCurrent ? 'font-bold' : ''} `} style={itemTextStyle}>
            {menuItem.icon.trim() == '' ? "" : <HeroIconOutline icon={menuItem.icon} className={itemIconClassName + ` ${isCurrent ? 'font-bold' : ''} `}></HeroIconOutline>}
            {menuItem.name}
          </Link>
        </Typography>
      ...
      {/* <Typography
        ...
        className="flex items-center gap-x-2 p-1 font-medium  transition-all duration-300 opacity-75 hover:opacity-100"
      >
        <svg
        ...
        <Link href="/#" className="flex items-center   " style={itemTextStyle}>
          HelloApp
        </Link>
      </Typography>
      <Typography
      ...
      >
        ...
        <a href="#" className="flex items-center  transition-all duration-1000" style={itemTextStyle}>
          Account
        </a>
      </Typography>
      <Typography
      ...
      >
        ...
        <Link
          href="/Photos" className="flex items-center  transition-all duration-1000" style={itemTextStyle}>
          Photos
        </Link>
      </Typography>
      <Typography
  ...
  return (<>
    ...
    <div className={`transition-all duration-1000 ${navClassName}`} style={navStyle}>
      <div className={` w-full  flex items-center justify-around px-2 `} >

        <div className={`hidden lg:flex ${navListClassName}`} style={navListStyle}>{navList}</div>
