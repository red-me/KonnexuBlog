...
const Page = ({ params }) => {
    ...
    const apps = {
        "helloapp":
            { url: `${NEXT_URL}/files/apps/hello`, scope: 'HELLO', module: 'HelloWorld' }
        ,
        "photos": {
    ...
    let [selectedApp, ...slugs] = appname;
    selectedApp = selectedApp.toLowerCase();


    const DynamicApp = dynamic(() => importRemote(apps[selectedApp]), { loading: () => Preloader, ssr: false })

    //const DynamicApp = dynamic(() => dynamicApps(selectedApp), { ssr: false })
    ...
    return (
        <React.Suspense  fallback={<div>Unable to Load...</div>}>
